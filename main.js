...
async function httpsGetHtml(hostname, urlPath) {
  const { gotScraping } = await import('got-scraping');
  const url = 'https://' + hostname + urlPath;
  const resp = await gotScraping({ url, timeout: { request: 15000 } });
  if (resp.statusCode >= 300 && resp.statusCode < 400 && resp.headers.location) {
    const loc = resp.headers.location;
    const u = new URL(loc, 'https://' + hostname);
    return httpsGetHtml(u.hostname, u.pathname + u.search);
  }
  if (resp.statusCode >= 200 && resp.statusCode < 300) return resp.body;
  throw new Error('HTTP ' + resp.statusCode);
}

async function downloadWithBypass(url) {
  const { gotScraping } = await import('got-scraping');
  const resp = await gotScraping({ url, timeout: { request: 15000 }, responseType: 'buffer' });
  if (resp.statusCode >= 300 && resp.statusCode < 400 && resp.headers.location) {
    return downloadWithBypass(resp.headers.location);
  }
  if (resp.statusCode >= 200 && resp.statusCode < 300) return resp.body;
  throw new Error('HTTP ' + resp.statusCode);
}
...
ipcMain.handle('fetch-avatar', async (event, { id }) => {
  try {
    const buf = await downloadFile('https://api.mcskin.me/head/' + encodeURIComponent(id) + '?size=128');
    return { success: true, dataUrl: 'data:image/png;base64,' + buf.toString('base64') };
  } catch (e) {
    try {
      const buf = await downloadFile('https://mc-heads.net/avatar/' + encodeURIComponent(id) + '/128');
      return { success: true, dataUrl: 'data:image/png;base64,' + buf.toString('base64') };
    } catch (e2) {
...
// ── Skin Checker (headless — NameMC HTTP) ───────────────────────

ipcMain.handle('scrape-namemc-skin', async (event, { ign }) => {
  try {
    const html = await httpsGetHtml('namemc.com', '/profile/' + encodeURIComponent(ign));

    const hashRegex = /data-id="([a-f0-9]{16})"/g;
    const hashes = [];
    const seen = new Set();
    let m;
    ...
    if (!hashes.length) return { success: false, error: 'No skins found on NameMC profile' };

    const activeHash = hashes[0];
    const skinBuf = await downloadWithBypass('https://s.namemc.com/i/' + activeHash + '.png');

    return {
      success: true,
      skinDataBase64: skinBuf.toString('base64'),
      skinHash: activeHash
    };
  } catch (e) {
    return { success: false, error: e.message };
...
// ── NameMC All-Skins Scraper (headless HTTP) ────────────────────

ipcMain.handle('scrape-namemc-all-skins', async (event, { ign }) => {
  try {
    const html = await httpsGetHtml('namemc.com', '/profile/' + encodeURIComponent(ign));
    ...
    for (const hash of hashes.slice(0, 27)) {
      try {
        const skinBuf = await downloadWithBypass('https://s.namemc.com/i/' + hash + '.png');
        const faceBuf = await sharp(skinBuf)
          .extract({ left: 8, top: 8, width: 8, height: 8 })
          .png()
          .toBuffer();

        skins.push('data:image/png;base64,' + faceBuf.toString('base64'));
      } catch {}
    }

    if (!skins.length) return { success: false, error: 'Failed to download any skins' };

    return { success: true, skins, count: skins.length };
  } catch (e) {
    return { success: false, error: e.message };
  }
});
